...
func (cq *CustomerQuery) QueryBills() *BillQuery {
	query := (&BillClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) WithBills(opts ...func(*BillQuery)) *CustomerQuery {
	query := (&BillClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withBills; query != nil {
		if err := cq.loadBills(ctx, query, nodes,
			...
			func(n *Customer, e *Bill) { n.Edges.Bills = append(n.Edges.Bills, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
