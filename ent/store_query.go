...
func (sq *StoreQuery) QueryBills() *BillQuery {
...
func (sq *StoreQuery) WithBills(opts ...func(*BillQuery)) *StoreQuery {
	query := (&BillClient{config: sq.config}).Query()
...
//
//	client.Store.Query().
//		GroupBy(store.FieldAccountName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (sq *StoreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Store, error) {
	...
	if query := sq.withBills; query != nil {
		if err := sq.loadBills(ctx, query, nodes,
			func(n *Store) { n.Edges.Bills = []*Bill{} },
			func(n *Store, e *Bill) { n.Edges.Bills = append(n.Edges.Bills, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StoreQuery) loadBills(ctx context.Context, query *BillQuery, nodes []*Store, init func(*Store), assign func(*Store, *Bill)) error {
	...
	query.Where(predicate.Bill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(store.BillsColumn), fks...))
	}))
